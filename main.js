...
function createConnectionGui() {
  ...
  if (isCreatingConnection) {
    ctx.fillText(`connection editor enabled`, 475, 485);
  } else {
    ctx.fillText(`hold [space] to add connections`, 475, 485);
  }
}
...
function createNodeGui(id) {
  let elem = document.createElement('div')
  elem.classList.add('node')
  elem.id = `node-${id}`
  elem.innerHTML = `
  <span class="nodeText"> name: <input id="nodeName-${id}" class="nodeInput nodeName" type="text" placeholder="CSC110"> </input></span>
  <span class="nodeText"> credits: <input id="nodeCredits-${id}" class="nodeInput nodeCredits" type="number" min="1" value="3"> </input></span>
  <span class="nodeText"> color: <input id="nodeColor-${id}" class="nodeInput nodeColor" type="color" value="#3dff57" > </input></span>`
  return elem
}
